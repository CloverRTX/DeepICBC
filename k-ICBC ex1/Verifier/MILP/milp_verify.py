...
def MILP_opt_unsafeCon_block(Bx):
    ...
    m.addConstr((x0[0] - 3) ** 2 + (x0[1] - 2) ** 2 <= 1)
...
def MILP_opt_initCon2(Bx, Col, fx_):
    ...
    x2 = m.addMVar((x0_len,), vtype=GRB.CONTINUOUS, name='x2', lb=-4, ub=4)
...
def MILP_opt_Indicator(Bx, Col, fx_):
    ...
    x3 = m.addMVar((x0_len,), vtype=GRB.CONTINUOUS, name='x3', lb=-4, ub=4)
...
def MILP_theNextPoint(m, x_pre, x_next, Col):
    ...
    m.addVar()
    x_pre_2_pow2 = m.addVar(vtype=GRB.CONTINUOUS, lb=0, ub=np.inf)
    x_pre_2_pow3 = m.addVar(vtype=GRB.CONTINUOUS, lb=-np.inf, ub=np.inf)
    m.addConstr(x_pre_2_pow2 == x_pre[1] * x_pre[1])
    m.addConstr(x_pre_2_pow3 == x_pre_2_pow2 * x_pre[1])
    ...
    m.addConstr(x0_dot == -x_pre[0] + x_pre[1] - x_pre[0] ** 2 - x_pre_2_pow3 + x_pre[0] * u)
    m.addConstr(x1_dot == -2 * x_pre[1] - x_pre[0] ** 2 + u)
