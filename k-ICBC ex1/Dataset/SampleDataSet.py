...
class Dataset():
    ...
    def __init__(self, path_Xi, path_Xu, path_X, path_X_bounded_area):
        ...
        with open(path_X_bounded_area, "r") as fp:
            data = list(csv.reader(fp))
            self.data_X_bounded_area = np.array(data).astype(float)
    ...
    def batchDivision(self, Xi_batch_size, Xu_batch_size, X_batch_size, X_bounded_area_batch_size):
        ...
        temp_data_X_bounded_area = self.arr_split(self.data_X_bounded_area, X_bounded_area_batch_size)


        num_max = max(len(temp_data_Xi), len(temp_data_Xu))
        num_max = max(num_max, len(temp_data_X))
        num_max = max(num_max, len(temp_data_X_bounded_area))
        ...
        num_X_bounded_area = len(temp_data_X_bounded_area)
        for i in range(num_max - num_X_bounded_area):
            temp = copy.deepcopy(temp_data_X_bounded_area[i])
            temp_data_X_bounded_area.append(temp)


        return temp_data_Xi, temp_data_Xu, temp_data_X, temp_data_X_bounded_area

    def fullBatch(self):
        return [copy.deepcopy(self.data_Xi)], [copy.deepcopy(self.data_Xu)], \
               [copy.deepcopy(self.data_X)], [copy.deepcopy(self.data_X_bounded_area)]
